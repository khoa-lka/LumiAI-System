...
const ADM_BAN_MOCK_USERS = [
  {
    accountId: "USR-001",
    email: "hoang.nguyen@las.vn",
    roleId: 1,
    status: "Active",
  },
  {
    accountId: "USR-002",
    email: "nhanvien1@las.vn",
    roleId: 3,
    status: "Active",
  },
  {
    accountId: "USR-003",
    email: "khachhang_scam@gmail.com",
    roleId: 3,
    status: "Banned",
  },
  {
    accountId: "USR-004",
    email: "tranthi.b@las.vn",
    roleId: 3,
    status: "Active",
  },
...
function filterAdminBanList() {
  const keyword = (document.getElementById("adm-ban-search").value || "")
    .trim()
    .toLowerCase();
  ...
  const filtered = _admBanUsersCache.filter((u) => {
    const status = String(u.status || "")
      .trim()
      .toLowerCase();
    const filterValue = String(statusFilter || "")
      .trim()
      .toLowerCase();
...
function changeRoleAction(userId, roleId, oldRoleId, selectElement) {
  ...
  API.updateUserRole(userId, newRoleId)
    .then((result) => {
      showAdminRoleSuccessPopup(result.oldRoleName, result.newRoleName);
...
function showBanPopup(type, title, message, onConfirm) {
  ...
  if (icon)
    icon.innerHTML =
      type === "success"
        ? '<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m5 13 4 4L19 7"/></svg>'
        : type === "error"
          ? '<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="8" x2="12" y2="13"/><line x1="12" y1="16.5" x2="12.01" y2="16.5"/><circle cx="12" cy="12" r="9"/></svg>'
          : '<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M9.5 9a2.5 2.5 0 0 1 4.9.8c0 1.7-2.4 1.8-2.4 3.5"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>';
...
const ADM_DB_MOCK_BACKUPS = [
  {
    ver: "LAS_DB_v1.0.2",
    date: "19/06/2026 23:59",
    size: "145.2 MB",
    type: "Tự động",
  },
  {
    ver: "LAS_DB_v1.0.1",
    date: "18/06/2026 15:30",
    size: "144.8 MB",
    type: "Thủ công",
  },
];
...
function renderAdminDbBackups() {
  ...
  API.getDbBackups()
    .then((backups) => {
      renderAdminDbRows(
        backups && backups.length ? backups : ADM_DB_MOCK_BACKUPS,
...
function restoreBackupAction(ver) {
  if (
    confirm(
      `Bạn có chắc chắn muốn phục hồi cơ sở dữ liệu về bản "${ver}"? Dữ liệu hiện tại có thể bị ghi đè.`,
    )
  ) {
    if (typeof API !== "undefined" && API.restoreBackup) {
      ...
      alert(
        "Chức năng phục hồi chưa được kết nối API thật (đang ở chế độ xem trước giao diện).",
      );
    }
  }
}
...
function createManualBackupAction() {
  if (typeof API !== "undefined" && API.createBackup) {
    API.createBackup()
      .then(() => {
        alert("Đã tạo bản sao lưu thủ công!");
        renderAdminDbBackups();
      })
      .catch((err) => alert("Lỗi khi tạo bản sao lưu: " + err.message));
  } else {
    alert(
      "Chức năng tạo bản sao lưu chưa được kết nối API thật (đang ở chế độ xem trước giao diện).",
    );
  }
}
