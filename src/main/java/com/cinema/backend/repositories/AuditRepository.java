...
public interface AuditRepository extends JpaRepository<Order1, Integer> {
    ...
    // 1. Tính tổng doanh thu từ vé xem phim (Chấp nhận cả COMPLETELY và COMPLETED)
    @Query(value = "SELECT COALESCE(SUM(final_amount), 0) FROM order1 " +
                   "WHERE CAST(created_date AS DATE) = CAST(:date AS DATE) " +
                   "AND order_status IN ('COMPLETELY', 'COMPLETED') AND showtime_id IS NOT NULL", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(t.ticket_id) FROM ticket t " +
                   ...
                   "AND t.ticket_status IN ('SOLD', 'BOOKED', 'SUCCESS', 'AVAILABLE')", nativeQuery = true)
    ...
    // 4. Tính tổng doanh thu F&B độc lập (Chấp nhận cả COMPLETELY và COMPLETED)
    @Query(value = "SELECT COALESCE(SUM(od.subtotal), 0) FROM orderdetail od " +
                   "JOIN order1 o ON od.order_id = o.order_id " +
                   "WHERE CAST(o.created_date AS DATE) = CAST(:date AS DATE) " +
                   "AND o.order_status IN ('COMPLETELY', 'COMPLETED') AND od.food_item_id IS NOT NULL", nativeQuery = true)
    BigDecimal getFnbRevenueByDate(@Param("date") String date);

    // 5. Đếm tổng số lượng sản phẩm F&B đã bán ra từ chi tiết hóa đơn (Chấp nhận cả COMPLETELY và COMPLETED)
    ...
    // 6. Tính tổng doanh thu trực tuyến (Chấp nhận cả COMPLETELY và COMPLETED)
    @Query(value = "SELECT COALESCE(SUM(final_amount), 0) FROM order1 " +
                   "WHERE CAST(created_date AS DATE) = CAST(:date AS DATE) " +
                   "AND order_status IN ('COMPLETELY', 'COMPLETED') AND payment_method NOT IN ('CASH', 'TIENMAT')", nativeQuery = true)
