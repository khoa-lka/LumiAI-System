...
public interface DashboardRepository extends JpaRepository<Order1, Integer> {
    ...
    // 1. Tính doanh thu hôm nay từ order1 thành công
    @Query(value = "SELECT COALESCE(SUM(final_amount), 0) FROM order1 WHERE CAST(created_date AS DATE) = CAST(GETDATE() AS DATE) AND order_status = 'COMPLETELY'", nativeQuery = true)
    BigDecimal getTodayRevenue();

    // 2. Đếm số vé bán ra hôm nay
    @Query(value = "SELECT COUNT(t.ticket_id) FROM ticket t JOIN order1 o ON t.order_id = o.order_id WHERE CAST(o.created_date AS DATE) = CAST(GETDATE() AS DATE) AND t.ticket_status IN ('SOLD', 'BOOKED', 'SUCCESS')", nativeQuery = true)
    ...
    // 4. Lấy top 5 phim có lượng vé đặt nhiều nhất
    @Query(value = "SELECT TOP 5 m.movie_id as movieId, m.title as title, COUNT(t.ticket_id) as ticketsCount, m.rating as rating " +
                   "FROM ticket t JOIN order1 o ON t.order_id = o.order_id JOIN showtime s ON o.showtime_id = s.showtime_id JOIN movie m ON s.movie_id = m.movie_id " +
    ...
    @Query(value = "SELECT m.genre as genreName, COUNT(t.ticket_id) as count FROM ticket t " +
                   "JOIN order1 o ON t.order_id = o.order_id JOIN showtime s ON o.showtime_id = s.showtime_id JOIN movie m ON s.movie_id = m.movie_id " +
