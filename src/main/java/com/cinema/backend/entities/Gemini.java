...
private String findMovieTitleInQuestion(String message) {
    ...
    for (Movie movie : movieRepository.findAll()) {
        ...
        String normalizedTitle =
                normalizeVietnamese(movie.getTitle());
...
private String buildExactShowtimeAnswer(String message) {
    LocalDate requestedDate =
            getRequestedShowtimeDate(message);

    String requestedMovieTitle =
            findMovieTitleInQuestion(message);
    ...
    List<Showtime> showtimes = showtimeRepository
            ...
            .filter(showtime ->
                    showtime.getStatus() == null
                    || showtime.getStatus()
                            .equalsIgnoreCase("ACTIVE")
            )
            .filter(showtime -> {
                String movieStatus =
                        showtime.getMovie().getStatus();

                return movieStatus == null
                        || !movieStatus.equalsIgnoreCase("hidden");
            })
            .filter(showtime ->
                    requestedMovieTitle == null
                    || showtime.getMovie()
                            .getTitle()
