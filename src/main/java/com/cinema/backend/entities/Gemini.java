 ...
 * MERGED VERSION
 * Base: Gemini.java
 * Updates: Gemini(1).java
 *
 * askGemini(String) remains available for legacy controllers and returns
 * Gemini-compatible JSON. askGemini(String, history) provides the upgraded
 * database-aware plain-text chatbot behavior.
 */
...
public class Gemini {
        ...
        /**
         * Giữ tương thích với controller cũ vốn gọi askGemini(String) và
         * mong nhận JSON theo cấu trúc candidates/content/parts của Gemini.
         *
         * Phần xử lý thật được chuyển sang overload mới để lịch chiếu, giá vé,
         * dữ liệu database và quy tắc hội thoại luôn đồng bộ.
         */
        public String askGemini(String userMessage) {
                try {
                        String answer = askGemini(
                                        userMessage,
                                        List.of());

                        ObjectNode response = objectMapper.createObjectNode();
                        ObjectNode candidate = response
                                        .putArray("candidates")
                                        .addObject();
                        ...
                        response.put(
                                        "error",
                                        "Hệ thống AI đang bận, bạn thử lại sau nhé.");

                        try {
                                return objectMapper.writeValueAsString(response);
                        } catch (Exception ignored) {
                                return "{\"error\":\"Hệ thống AI đang bận, bạn thử lại sau nhé.\"}";
                        }
                }
        }
